...
class advGAN():
    ...
    def _create_placeholder(self):
        ...
        # the target images, which has the different label. We can also conduct
        # target attack later
        ...
        self.labels = tf.placeholder(
            tf.float32, [None, self.opts.label_dim], name="label")
    ...
    def _GAN_model(self, images, fake_images, real_images, g_x):
        ...
        L2_norm = tf.reduce_mean(tf.reduce_sum(
            tf.square(fake_images - images), [1, 2, 3]))
        L1_norm = tf.reduce_mean(tf.reduce_sum(
            tf.abs(fake_images - images), [1, 2, 3]))
        ####
    ...
    def reranking(self, predict_labels, target_labels):
        ...
        other = tf.reduce_max((1 - target_labels) *
                              predict_labels - target_labels * 1000, 1)
    ...
    def _adversial_g_loss(self, adv_fake_predict_labels, labels):
        adv_G_loss = tf.reduce_mean(
            self.reranking(adv_fake_predict_labels, labels))
        return adv_G_loss
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('evagan'):
            ...
            self.fake_images_sample_flatten = tf.reshape(
                self.fake_images_sample, [-1, 28 * 28])
            self.fake_images_correct = tf.reshape(
                self.fake_images, [-1, 28 * 28]) * 0.5 + 0.5

            self.fake_images_sample_sum = tf.summary.image(
                "fake_images", self.fake_images_sample)
            ...
            self.pre_g_loss_sum = tf.summary.scalar(
                "pre_G_loss", self.pre_G_loss)
            self.pre_g_totoal_loss_sum = tf.summary.scalar(
                "pre_G_loss_total", self.G_loss)
            ...
            self.hinge_loss_sum = tf.summary.scalar(
            ...
            self.G_loss_add_adv = G_lambda * G_loss + ld * self.adv_G_loss + \
                L1_lambda * L1_norm + L2_lambda * L2_norm + hinge_lambda * hinge_loss
            self.adv_g_loss_sum = tf.summary.scalar(
                "adv_G_loss", self.adv_G_loss)
            self.g_loss_add_adv_sum = tf.summary.scalar(
                "G_loss_total", self.G_loss_add_adv)
            self.adv_accuracy_sum = tf.summary.scalar(
                "adv_accuracy", self.adv_accuracy)
            self.g_loss_add_adv_merge_sum = tf.summary.merge([
                self.adv_g_loss_sum, self.l1_norm_sum, self.l2_norm_sum,
                self.g_loss_add_adv_sum, self.pre_g_loss_sum, self.hinge_loss_sum])

            t_vars = tf.trainable_variables()
            ##
            self.d_vars = [
                var for var in t_vars if 'd_' in var.name and 'evagan' in var.name]
            self.g_vars = [
                var for var in t_vars if 'g_' in var.name and 'evagan' in var.name]

            # self.adv_d_vars = [var for var in t_vars if 'adv_' in var.name and 'evagan' in var.name]
            D_pre_opt = tf.train.AdamOptimizer(self.lr)
            D_grads_and_vars_pre = D_pre_opt.compute_gradients(
                self.D_loss, self.d_vars)
            D_grads_and_vars_pre = [
                (tf.clip_by_value(gv[0], -1.0, 1.0), gv[1]) for gv in D_grads_and_vars_pre]
            self.D_pre_train_op = D_pre_opt.apply_gradients(
                D_grads_and_vars_pre)

            # G loss without adversary loss
            G_pre_opt = tf.train.AdamOptimizer(self.lr)
            G_grads_and_vars_pre = G_pre_opt.compute_gradients(
                self.G_loss, self.g_vars)
            G_grads_and_vars_pre = [
                (tf.clip_by_value(gv[0], -1.0, 1.0), gv[1]) for gv in G_grads_and_vars_pre]
            self.G_pre_train_op = G_pre_opt.apply_gradients(
                G_grads_and_vars_pre)
            # G loss with adversary loss
            G_opt = tf.train.AdamOptimizer(self.lr)
            G_grads_and_vars = G_opt.compute_gradients(
                self.G_loss_add_adv, self.g_vars)
            G_grads_and_vars = [(tf.clip_by_value(gv[0], -1.0, 1.0), gv[1])
                                for gv in G_grads_and_vars]
    ...
    def discriminator(self, image, y=None, reuse=False):
        ...
        with tf.variable_scope("discriminator") as scope:
            ...
            h2 = lrelu(self.d_bn2(conv2d(h1, self.df_dim *
                                         4, d_h=1, d_w=1, name='d_h2_conv')))
            # h2 is (32x 32 x self.df_dim*4)  4 x 4 x df_dim*4
            h3 = lrelu(self.d_bn3(conv2d(h2, self.df_dim *
                                         8, d_h=1, d_w=1, name='d_h3_conv')))
            h4 = linear(tf.reshape(
                h3, [self.opts.batch_size, -1]), 1, 'd_h3_lin')
    ...
    def patch_discriminator(self, image, y=None, reuse=False):
        ...
        with tf.variable_scope("discriminator") as scope:
            ...
            h3 = lrelu(self.d_bn3(conv2d(h2, self.df_dim *
                                         8, d_h=1, d_w=1, name='d_h3_conv')))
            h4 = lrelu(self.d_bn5(
                conv2d(h3, 1, d_h=1, d_w=1, name='d_h5_conv')))
    ...
    def generator(self, image, y=None):
        with tf.variable_scope("generator") as scope:
            ...
            e2 = self.g_bn_e2(
                conv2d(lrelu(e1), self.gf_dim * 2, name='g_e2_conv'))
            # e2 is (8 x 8 x self.gf_dim*2) 7x 7
            e3 = self.g_bn_e3(
                conv2d(lrelu(e2), self.gf_dim * 4, name='g_e3_conv'))
            # # e3 is (4 x 4 x self.gf_dim*4)
            e4 = self.g_bn_e4(
                conv2d(lrelu(e3), self.gf_dim * 8, name='g_e4_conv'))
            # # e4 is (2 x 2 x self.gf_dim*8)
            e5 = self.g_bn_e5(
                conv2d(lrelu(e4), self.gf_dim * 8, name='g_e5_conv'))
            ...
            self.d3, self.d3_w, self.d3_b = deconv2d(tf.nn.relu(d2),
                                                     [self.batch_size, 7, 7, self.gf_dim * 2], name='g_d3', with_w=True)
            d3 = tf.nn.dropout(self.g_bn_d3(self.d3), 0.5)
            d3 = tf.concat([d3, e2], 3)
            # d3 is (8 x 8 x self.gf_dim*2*2)

            self.d4, self.d4_w, self.d4_b = deconv2d(tf.nn.relu(d3),
                                                     [self.batch_size, 14, 14, self.gf_dim], name='g_d4', with_w=True)
            d4 = self.g_bn_d4(self.d4)
            d4 = tf.concat([d4, e1], 3)
            # d4 is (16 x 16 x self.gf_dim*8*2)

            self.d5, self.d5_w, self.d5_b = deconv2d(tf.nn.relu(d4),
                                                     [self.batch_size, 28, 28, self.output_c_dim], name='g_d5', with_w=True)
            # return tf.clip_by_value( tf.clip_by_value( tf.nn.tanh(self.d5) ,
            # -0.6, 0.6) + image, -1.0, 1.0) , tf.nn.tanh(self.d5)
            return tf.clip_by_value(
                self.opts.c * tf.nn.tanh(self.d5) + image, -1.0, 1.0), tf.nn.tanh(self.d5)
