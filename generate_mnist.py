...
def train():
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        iteration = 0
        # Loading positive and negative classifiers.
        # TODO: Change MNISTModel class.
        model_store = opt.model_restore
        whitebox_model = MNISTModel(model_store, sess)
        positive_model = MNISTModel('model/odd_even', sess)
        negative_model = MNISTModel('models/MNIST', sess, 2)
        # Loading initial advGAN.
        ...
        while iteration < 2000:
            ...
            if iteration != 0 and iteration % opt.losses_log_every == 0:
                print(
                    "loss(D, G, pre_G_loss,  adv_G, L1_norm, L2_norm, hinge_loss ): ",
                    D_loss,
                    G_loss,
                    pre_G_loss,
                    adv_G_loss,
            ...
            if (iteration != 0 and iteration % opt.save_checkpoint_every == 0):
                checkpoint_path = os.path.join(
                    opt.checkpoint_path, 'model.ckpt')
                test_loader = Dataset2(
                    test_data,
                    test_label,
                    targeted=True,
                    source_label=source_label)
                ...
                for i in range(test_iter_num):
                    ...
                    s_imgs, s_label, _ = test_loader.next_batch(
                ...
                if min_adv_accuracy > test_adv_accuracy:
                    ...
                    test_out_str = "test total accuracy {}, adv accuracy {}".format(
                        str(test_accuracy), str(test_adv_accuracy))
                    print(test_out_str)
